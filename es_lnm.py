...
#应用标题
st.title('A machine learning-based predictive model for predicting lymph node metastasis in patients with Ewing’s sarcoma')

# conf
st.sidebar.markdown('## Variables')
#Age = st.sidebar.slider("Age", 1, 99, value=25, step=1)
Race = st.sidebar.selectbox('Race',('White','Black','Other'),index=0)
...
thyroid_train['Lymph.node.metastases'] = thyroid_train['Lymph.node.metastases'].apply(lambda x : +1 if x==1 else 0)
#thyroid_test = pd.read_csv('test.csv', low_memory=False)
#thyroid_test['Lymph.node.metastases'] = thyroid_test['Lymph.node.metastases'].apply(lambda x : +1 if x==1 else 0)
#features = ['T','N','Sex','surgery','Bone.metastases','Radiation']
features = ['T','M','surgery','Lung.metastases','Race']#
target = 'Lymph.node.metastases'
#处理数据不平衡
...
sp = 0.5
#figure
is_t = (RF.predict_proba(np.array([[T,M,Race,surgery,Lung_metastases]]))[0][1])> sp
prob = (RF.predict_proba(np.array([[T,M,Race,surgery,Lung_metastases]]))[0][1])*1000//1/10
